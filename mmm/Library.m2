...
IMPLEMENTATION MODULE Library;
   ...
   PROCEDURE ScanLibrary(directory: ARRAY OF CHAR);
	 ...
	 LOOP
	    Fread(fp, dirbuf);
	    IF Done THEN
	       IF (dirbuf[0] <> 0C) OR (dirbuf[1] <> 0C) THEN
		  fref := FileRef(ADR(dirbuf[2]));
		  dirfile := fref^;
   ...
   BEGIN
      IF NOT Fopen(fp, directory, read, (* buffered = *) TRUE) THEN
