IMPLEMENTATION MODULE Printf;
	 ...
	 RETURN cp^;
      ...
      PROCEDURE AddToReal (add : ADDRESS; sz : CARDINAL) : REAL;
         ...
         VAR 
            real : RECORD 
		      CASE : BOOLEAN OF
			 TRUE : bytes : ARRAY[0..SIZE(REAL)-1] OF BYTE;
	 	      |	 FALSE: real  : REAL;
		      END;
		  END;
      ...
      BEGIN 
         real.real := 0.0;
         Copy(add,ADR(real.bytes),sz,SIZE(REAL));
         RETURN real.real;
      END AddToReal;
   ...
   PROCEDURE Printf(output : Long; no: CARDINAL; VAR fmt: ARRAY OF CHAR;
	 ...
	 MaxDp  = 15; (* maximum number of decimal places (real conversions) *)
	 IncDp  =  7; (* additional space for WriteFloat *)
