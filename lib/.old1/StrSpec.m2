IMPLEMENTATION MODULE StrSpec;		(* gsk 1/85 *)
   ...
   BEGIN
      ...
      IF ( number = 0 ) OR ( position > targetLen ) THEN
	 RETURN
      ELSE
	 index := 0;
	 WHILE ( index <= position ) AND ( index <= targetLen ) DO
	    target [ index ] := target [ index ];
	    INC ( index )
	 END;
	 index := position;
	 WHILE  ( index <= targetLen - number )
		AND ( index + number <= targetLen ) DO
   ...
   BEGIN
	 ...
	 RETURN HIGH ( source ) + 1
      ELSE
	 index := 0;
	 WHILE index <= sourceLen - searchLen DO
	    WHILE ( index <= sourceLen - searchLen )
			AND ( source [ index ] <> search [ 0 ] ) DO
	       INC ( index )
	    END;
	    position := index;
	    lauf := 0;
