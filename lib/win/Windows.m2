...
IMPLEMENTATION MODULE Windows;
   ...
   *)
   ...
   BEGIN
	 ...
	 END;
	 OpenScreenTI(scr, out, baudrate, tinfo^.term);
	 ClearScreen(scr);
	 ...
	 ELSE
	    ...
	    atset := AttributeSet{};
	 END;
	 AllocateScreen(screen, tinfo^.lines, tinfo^.columns);
	 ndelay := FALSE;
	 cursorvis := normal;
	 fkeysenabled := FALSE;
	 x := 0; y := 0;
   ...
   BEGIN
	 ...
	 oldx := x; oldy := y;
	 ClearScreen(scr);
	 FOR l := 0 TO tinfo^.lines-1 DO
	    SetCursor(scr, l, 0); lastx := l; lasty := 0;
	    FOR c := 0 TO tinfo^.columns-1 DO
	       IF (l <> tinfo^.lines-1) OR (c <> tinfo^.columns-1) THEN
		  WITH screen^[l]^[c] DO
   ...
   BEGIN
      WITH dev^ DO
	 IF (line >= tinfo^.lines) OR (column >= tinfo^.columns) THEN
	    Error; RETURN 0C
	 ELSE
   ...
   BEGIN
	 ...
	 IF useats THEN
	    ResetAttributes(atinfo);
	 END;
	 MoveCursor(scr, (* from *) x, y, (* to *) tinfo^.lines-1, 0);
   ...
   BEGIN
      Done := TRUE;
      InitWindow(win, dev, dev^.tinfo^.lines, dev^.tinfo^.columns);
   END CreateWindow;
...
(* END HP *)
   ...
   BEGIN
      RETURN (ch >= fkbase) AND (ch <= CHR(ORD(fkbase) + ORD(MAX(FunctionKey))))
   END IsFunctionKey;
