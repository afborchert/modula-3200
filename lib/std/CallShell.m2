IMPLEMENTATION MODULE CallShell;
   ...
   PROCEDURE CatchSIGINT;
   BEGIN
      IF NOT Signal(SIGINT, CatchSIGINT) THEN END;
   END CatchSIGINT;

   PROCEDURE CatchSIGQUIT;
   BEGIN
      IF NOT Signal(SIGQUIT, CatchSIGQUIT) THEN END;
   END CatchSIGQUIT;
   ...
   BEGIN
      ...
      IF child = 0 THEN (* son *)
         ...
         Exit(ExecFailed);
      ELSE (* father *)
         ign := Signal(SIGINT, CatchSIGINT); intproc := old;
         ign := Signal(SIGQUIT, CatchSIGQUIT); quitproc := old;
