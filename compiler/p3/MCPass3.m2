(* Modula-2 Multipass-Compiler    XELOS / Concurrent 3220 *)
(* (c) Universitaet Ulm, Sektion Informatik,   D-7900 Ulm *)
(*%c20*)
MODULE MCPass3;     (* LG *)
                    (* REV AFB 12/83 :  new string handling *)
                    (* REV AFB 1/84  :  no CODE procedures *)
                    (*                  new standard procedures *)
                    (* REV AFB 5/84  :  no space for with-addresses *)
...
(* $T+ *)
   ...
   MODULE ExpressionSystem;
      ...
      FROM MCBase IMPORT noprio, Idptr, Stptr, Idclass, Idset, Structform,
         ...
         longmaxint, longmaxcard, xelos, modrev2;
	 ...
	 BEGIN
	    ...
	    END;
            ...
            IF (t1p = t2p) OR (t1p = NIL) OR (t2p = NIL) THEN 
               RETURN TRUE 
            END;
	    IF IsReal(t1p) AND IsReal(t2p) THEN RETURN TRUE END; (* DEBUG *)
	    IF (t1p^.form IN typesettypes) OR (t2p^.form IN typesettypes) THEN
	 ...
	 END Compatible;
	    ...
	    (* assignment compatibility *)
         ...
         BEGIN 
            RETURN ExprComp(at1,at2) OR FAmong(at1.atp,incar) AND FAmong(at2.
               atp,incar) OR modrev AND IsCharComp(at1) AND IsCharComp(at2)
         END AssignComp;
			...
			IF FAmong(tp,scalars) THEN 
			   ...
			   WITH at2 DO 
				 ...
				 IF (at1.mode = constm) AND
				    ...
				    fval := at1.aval.value;
				    lval := at2.aval.value;
				    IF FAmong(tp, Stset{ints,longints}) THEN 
		  ...
		  END ConstBigSetConstructor;
			...
			IF (styp <> NIL) AND Compatible(styp, ctyp) THEN
                           ...
                           ELSE
                              v2 := smax;
                           END;
                           IF (v2 < v1) OR (v1 < smin) OR (v2 > smax) THEN 
                              ErrorLS(98);
		     ...
		     (* skip semantical incorrect elements of factor *)
                  ...
                  PROCEDURE StFuncCheck(ip: Idptr; VAR at : Attribut);
                     CONST 
				       ...
				       words,subranges,longreals,reals,pointers,
				       sets,proctypes,arrays,records,
				       bigsets,longints,longcards,
				       setoftypes};
                     TYPE
                        Kludge =
                           RECORD
      ...
      BEGIN                             (* Expression *)
         ...
         IF (sy >= eql) AND (sy <= insy) THEN 
            ...
            ELSIF ExprComp(at,lat) OR AddressComp(at,lat) THEN 
               ...
               CASE op OF 
                 eql,neq : 
                     forms := Stset{sets,bigsets,pointers,reals,longreals} + scalars;
                     IF modrev THEN
                        INCL(forms, hides);
                     END;
               | geq,leq : 
