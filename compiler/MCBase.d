...
DEFINITION MODULE MCBase; (* LG / CHJ / AFB *)
   ...
   CONST
    ...
    adc = 4;                         (* address constant *)
    oneword = adc * onebyte;         (* storage size of one word *)
    ...
    procmarkspace = 3 * oneword;     (* space used for procedure mark *)
    ...
    modnamlength = 8;                (* maximal length for module identifiers *)
    xmodnamlength = 24;              (* module id length under XELOS *)
    maxprio = 15;                    (* maximal priority allowed *)
   ...
   TYPE
	 ...
	 RECORD
      ...
      XModnamarr = ARRAY [0..xmodnamlength-1] OF CHAR;
      Levrange = [0..levmax]; 

      (* some changes for UNIX: *)
      (* argcf & argvp for command line arguments *)
      (* uxf for having a interface to UNIX *)
      (* uxsf: UNIXSIGNAL *)
      (* uxff: UNIXFORK *)
		   ...
		   TRUE:
		       ...
		       | mods:
				   ...
				   globvarp: Idptr; (* global variables *)
                                   modnum: CARDINAL;
                                   modulekey: Keyarr;
				   CASE (* xelos *) : BOOLEAN OF
				   | FALSE: identifier: Modnamarr;
				   | TRUE: xidentifier: XModnamarr;
				   END;
   ...
   VAR 
      ...
      xelos      : BOOLEAN; (* if on: unix system v operating system *)
