...
MODULE MCPass2; (* LG / UA *) (* REV AFB 8/83 *)
                              (* REV AFB 1/84 *)
                              (* REV AFB 6/84: Modula-2 Revision *)
                              (* REV AFB 1/85: big sets *)
			      (* REV AFB 9/85: bug of m2m-compiler fixed *)
			      (*               see ImportList *)
  ...
  FROM MCBase IMPORT Idptr, Stptr, Structform, Stset, Idclass, Idset,
    ...
    procnumber, modnamlength, maxprio, noprio, Symbol, adc, onebyte,
    oneword, doubleword, procmarkspace, BitsPerWord, modrev, maxcard,
    longword, xmodnamlength, xelos;
  ...
  END InitId;
   ...
   PROCEDURE Align(VAR addr: CARDINAL);
      VAR incr: CARDINAL;
   BEGIN
      IF addr MOD adc <> 0 THEN
         incr := adc - addr MOD 4;
         IF CARDINAL(-1) - addr >= incr THEN
            INC(addr, incr);
         ELSE
            ErrorLS(100);
         END;
   ...
   CONST byteSize = Stset{chars, bytes};
      ...
      PROCEDURE ConstSimpleExpression(VAR cf: Constform);
	    ...
	    BEGIN 
		  ...
		  IF NOT FAmong(argcf.ctp, scalars +
			 Stset{reals, longreals, sets}) OR
		     NOT FAmong(cf.ctp, scalars +
			 Stset{reals, longreals, sets}) THEN
		     ErrorLS(139);
		     cf.ctp := NIL;
		     cf.cval.value := 0;
		  ELSE
