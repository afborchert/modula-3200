...
(* $T- *)
   ...
   IMPORT StdIO, Storage, Conversions, MCBase, MCSymFileDefs, MCStop,
      SysPerror, FtdIO, MCHalfword;

   MODULE Arguments;			(* work up command line arguments *)

      FROM StdIO IMPORT stderr;
      FROM FtdIO IMPORT FwriteString, FwriteLn;
      FROM MCStop IMPORT Stop;
      FROM MCBase IMPORT loading;
      EXPORT ascName, symName;
      ...
      BEGIN
         ...
         FwriteString(stderr, msym);
         FwriteString(stderr, " storage asc out");
         FwriteLn(stderr);
         Stop(1);
      END Usage;
               ...
               WITH sp^ DO 
                  IF form = arrays THEN (* string constant *)
                     SymPutStr(val.svalue^.valentry,ixp^.max + 1);
                                        (* addr,length *)
                  ELSIF (form = reals) OR (form = longreals) THEN 
                     SymPutS(realconstSS);
                     rconv.rc := val.rvalue^;
