...
MODULE MCPass1;                (* SEK / LG *)
                               (* REV AFB 3/84 *)
		               (* REV AFB 11/84: better error recovery *)

(* $T- *)

   IMPORT MCP1Public;                   (* for initialization *)

   FROM Storage IMPORT EndStorage;
   ...
   MODULE SymsetHandling;
      ...
      EXPORT Symset, Set1, Set2, Set3, InSet, InclSet, AddSet, SubSet, GenSet1
         , GenSet2, GenSet3, s0;
...
BEGIN                                   (* MCPass1 *)
   ...
   EndStorage;
   IF SymFilesMissing THEN 
      Stop(2);                          (* abort the compilation *)
   ELSIF ErrorsFound THEN 
      Stop(1);
   END;
END MCPass1. 
