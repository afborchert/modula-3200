...
static char * regnames[] = { "ps", "pc", "rf", "base", "top", "rc", "rb", "ra",
...
{	char cmd;
	...
	{	cmd = getcmd();
		...
		/*	case 'i':	print_instr(); break; */
			...
			case 'C':	cont(1); mdbmode(); break;
			case 'D':	breakpoint('D'); break;
			case 'I':	cont(0); mdbmode(); break;
			case 'M':	p_switch('M'); break;
			case 'P':	p_switch('P'); break;
			case 'R':	run(); mdbmode(); break;
...
{	unsigned int ins;
	...
	{	if (mod = getmod(idot))
		{	printf("[%s", mod->m_name);
			...
			printf("[#%x]", idot);
		putchar('\t');
		printins(space, ins); puts(tobeprinted);
		idot += idotinc;
	}
...
{	tree * t;
	...
	{	if (mode != 'd')
			cmderror("Only in D-window allowed");
		t = (*dot)->v_node;
		if (! t)
			cmderror("Empty node");
		addr = getvaraddr(t);
