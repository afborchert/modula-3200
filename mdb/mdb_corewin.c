...
#define	SCREEN 23		/* #ifdef LINE: start cwin at addr-SCREEN*4 */
...
view ** corewin(var, mod, defaddr, defsize, addrformat, format)
	...
	address defaddr;
	unsigned defsize;
	char addrformat, format; /* only #ifdef LINE: == 'd', 'o', 'x' or 'c' */
{	view ** cwin;	/* to be returned */
...
#ifndef LINE
	/*
	 *	noscreen on: rebuilding C-window (sub process stops)
	 */
	if (! noscreen)
	{	if (defaddr && defsize || getvaraddr(var))
			addtext = "or press <return>";
		else
			addtext = "";
		message("Enter octal address and # fullwords %s",
		   addtext);
		len = 0;
	}
	else
	...
	sprintf( (*ptr)->v_line, "core dump at %oB, %d fullwords",
		addr, len);
	(*ptr)->v_mod = NULL;
	(*ptr)->v_node = NULL;
	++ptr;
	...
	(*ptr)->v_mod = NULL;  (*ptr)->v_node = NULL;
	strcpy( (*ptr)->v_line, "");
#else
	--ptr;
#endif LINE
	...
	while ( len-- ) {
		...
		{	char fmt[32];
			...
			if (format == 'a')
				...
				sprintf((*ptr)->v_line, fmt, addr,
					format=='c'? ascii(value): value);
		}
#else
			sprintf( (*ptr)->v_line, "[%8oB] %8xH %11oB %10d %s",
					addr,
					value, value, value,
					ascii(value) );
#endif
...
{	address addr;
	...
	addr = var->t_address;
	if (version == 1)
		addr *= 4;		/* multiply with address constant */
